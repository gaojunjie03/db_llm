...
def insert_into_milvus(milvus_collection_name,milvus_collection_description,ds_type,database_ip,database_port,database_name,database_schema,database_username,database_password,table_infos,field_infos):
    if milvus_collection_name is None or milvus_collection_name=="":
        return gr.update(value="❌ 请输入入库表名！",visible=True),milvus_collection_name,milvus_collection_description,ds_type,database_ip,database_port,database_name,database_schema,database_username,database_password,table_infos,field_infos,[],gr.update(visible=True)
    if milvus_collection_description is None or milvus_collection_description=="":
        return gr.update(value="❌ 请输入描述用途！",visible=True),milvus_collection_name,milvus_collection_description,ds_type,database_ip,database_port,database_name,database_schema,database_username,database_password,table_infos,field_infos,[],gr.update(visible=True)
    ...
    try:
        with open_milvus() as db:
            ...
            for start in range(0, len(table_column_description_df), batch_size):
                ...
                description_array=batch["description"].tolist()
                description_array=["" if d == None or d == "null" else d for d in description_array]
                embeddings = constants.sentence_model.predict(description_array)
                data = [
                    ...
                    for table_real_name, embedding, columns,description in zip(
                        batch["table_real_name"].to_numpy(), embeddings, batch["columns"].to_numpy(),description_array
                    )
                ]
        ...
        return gr.update(value="✅ 入库成功！", visible=True),gr.update(value="",visible=False),gr.update(value="",visible=False),1,"",0,"","","","",[],[],[],gr.update(visible=False)
    except Exception as e:
        logging.error(traceback.format_exc())
        return gr.update(value="❌ 入库失败！", visible=True),milvus_collection_name,milvus_collection_description,ds_type,database_ip,database_port,database_name,database_schema,database_username,database_password,table_infos,field_infos,[],gr.update(visible=True)
...
def inner_get_ds_collection_infos():
...
def change_database_info(ds_collection_info_id,all_ds_collection_info):
    if ds_collection_info_id == 0:
        return gr.update(value=1,interactive=True),"",0,"","","","",gr.update(value="",visible=False),gr.update(value="",visible=False),None,[],[],[],gr.update(visible=False),gr.update(visible=True),gr.update(visible=False)
    ...
    df_outer = df[["table_real_name", "description", "columns"]].copy()
    df_outer = df_outer.rename(columns={"table_real_name":"数据库表名","description":"描述"})
    return gr.update(value=ds.TYPE,interactive=False),ds.IP,ds.PORT,ds.DATABASE_NAME,ds.SCHEMA,ds.USERNAME,ds.PASSWORD,gr.update(value=collection_name,visible=True),gr.update(value=description,visible=True),result_ds_collection_info,df_outer[["数据库表名","描述"]], df_outer["columns"].tolist(),df_outer["数据库表名"].tolist(),gr.update(visible=True),gr.update(visible=False),gr.update(visible=True)
    
def delete_ds_collection_info(result_ds_collection_info):
    with open_milvus() as db:
        db.client.drop_collection(collection_name=result_ds_collection_info.collection_name)
    with open_sqlite(need_commit=True) as db:
        db.session.delete(result_ds_collection_info)
    rows=inner_get_ds_collection_infos()
